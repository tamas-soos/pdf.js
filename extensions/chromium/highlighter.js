...
function initialize() {
  console.log("highlighter initialize");
  ...
  function onSelectionChange(e) {
    ...
    if (!options.isWindowLocationValid(window.location)) {
      return;
    }
    if (!options.areKeysPressed(pressedKeys)) {
      return;
    }
  ...
  function highlight(startTime) {
    ...
    function highlightOccurrences(textNode) {
      ...
      if (!options.isAncestorNodeValid(textNode.parentNode)) {
        return;
      }
      ...
      if (!isUsersSelection) {
        ...
        if (parent) {
          parent.replaceChild(highlightedNode, trimmedTextNode);
        }
